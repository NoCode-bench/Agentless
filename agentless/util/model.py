...
class OpenAIChatDecoder(DecoderBase):
    ...
    def codegen(
        ...
        if batch_size >= 8:
            ret_list = request_chatgpt_with_batch(config, self.logger)
            ...
            for ret in ret_list:
                if not ret:
                    continue
                for choice in getattr(ret, "choices", []):
...
class PDeepSeekChatDecoder(DecoderBase):
    def __init__(self, name: str, logger, **kwargs) -> None:
        super().__init__(name, logger, **kwargs)

    # 单条请求的逻辑抽到一个私有方法里，便于复用
    def _one_sample(self, message: str) -> dict:
        config = create_chatgpt_config(
            message=message,
            max_tokens=self.max_new_tokens,
            temperature=self.temperature,
    ...
    def codegen(
        ...
        trajs = []
        if num_samples==1:
            trajs.append(self._one_sample(message))
        # max_workers 可以按需调节；API 有限速时不要开太大
        else:
            with ThreadPoolExecutor(max_workers=num_samples) as executor:
                futures = [
                    executor.submit(self._one_sample, message)
                    for _ in range(num_samples)
                ]
...
def make_model(
    ...
    elif backend == "par_deepseek":
        return PDeepSeekChatDecoder(
            name=model,
            logger=logger,
            batch_size=batch_size,
            max_new_tokens=max_tokens,
